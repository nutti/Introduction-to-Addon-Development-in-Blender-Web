...
ユーザー・プリファレンスに追加された全てのアドオンの設定情報は、```context.user_preferences.addons``` にパッケージ名をキーとした辞書型で保存されています。本節のサンプルでは自分自身のアドオンの設定情報を取得するため、```context.user_preferences.addons[__name__]``` で取得することができます。仮にアドオンが複数のファイルで構成されている場合は、```context.user_preferences.addons[__package__]``` のように指定することでアドオンの設定情報を取得できます。
