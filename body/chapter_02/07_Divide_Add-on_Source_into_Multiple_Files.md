...
上記のコードを簡単に説明します。
最初の if 文では、 ```bpy``` モジュールが既にインポートされているかを判定します。
...
もし ```bpy``` モジュールのインポートをモジュール読み込み処理の前に書いてしまうと、最初の if 文で真と判定されてしまいます。

### bl_info 変数の作成

```__init__.py``` には、 ```bl_info``` 変数を記載する必要があります。
記載内容は、 [2.1節](../chapter_02/01_Basic_of_Add-on_Development.md) と同じですので、ここでは説明を省略します。
