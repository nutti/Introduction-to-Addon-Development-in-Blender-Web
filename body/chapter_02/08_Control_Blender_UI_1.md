...
```bl_label``` には、ツール・シェルフのタブに表示される文字列を指定します。メンバ変数 ```bl_space_type``` には登録先のエリアを指定します。本節のサンプルでは、*3Dビュー* エリアに登録することを考えて ```VIEW_3D``` を指定しています。
...
```bl_context``` はパネルクラス時に指定することが可能なメンバ変数で、**指定したコンテキストである時のみパネルの描画処理を実行します。** ```bl_context``` には以下のような値を設定することができます。
...
BlenderのUIを変更すると聞くと難しそうに思えますが、Pythonさえ理解できていればなんとかなりそうだと、ここまでの説明を読んだ方であれば思えてきたのではないでしょうか？なお、UIの説明を行っている [2-9節](09_Control_Blneder_UI_2.md) と [2-10節](10_Control_Blender_UI_3.md) の内容も一緒に理解すれば、Blenderで実現可能な大半のUIを自由に扱えるようになったと言ってよいと思います。
