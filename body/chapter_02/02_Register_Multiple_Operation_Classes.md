...
今回新たに解説する内容として、複数のオペレータクラスを登録する処理があります。
...
```bl_idname``` はオペレータクラスごとに固有の文字列でなければならないので、重複しないように気をつけてください。
重複しないようにするための方法は後ほど紹介するので、気になった方は参考にしてみてください。

### コンソール・ウィンドウへの出力

オペレータクラスの ```execute()``` 関数では、メニューが実行された時にコンソール・ウィンドウへメッセージを出力する処理があります。
...
このように、オペレータクラスを登録すると ```bpy.ops.<オペレーションクラスのbl_idname>``` に登録されることが理解できたのではないでしょうか。
