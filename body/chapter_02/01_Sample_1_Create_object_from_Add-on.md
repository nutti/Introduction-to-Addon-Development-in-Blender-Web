...
|```COMMUNITY```|コミュニティ|Blenderが公式にサポートしていないアドオンです。サポートレベルが *Contrib* あたは *External* のアドオンに対して設定します。|
|```TESTING```|テスト中|テスト中のアドオンに対して設定します。機能が不完全な場合やアドオンをテスト中の場合に設定します。|

*ユーザ設定* ウィンドウでは、サポートレベルに応じてインストールされたアドオンをフィルタリングして表示することが可能です。
サンプルのアドオンは ```support``` に ```TESTING``` を設定しています。
*テスト中* のボタンのみ選択することで、 ```support``` に ```TESTING``` が設定されたアドオンのみ表示し、サンプルのアドオンを素早く検索できるようにしました。
今後紹介するサンプルのアドオンでも同様に ```support``` に ```TESTING``` を設定し、サンプルのアドオンをすぐに見つけられるようにしました。
...
bl_infoがなくても、ソースコードを実行し機能を利用する方法はあります。
しかし、アドオンを作る場合はbl_infoを設定することが、Blender公式のアドオン開発のガイドラインでも
</div>
