...
アドオンの動作を確認してから解説した方が理解が深まると思いますので、解説を始める前に作成したアドオンを使ってみましょう。
...
Blenderのアドオンであることを、Blender本体に認識させるためには ```bl_info``` と呼ばれる変数を作成する必要があります。```bl_info``` はディクショナリ型で以下のようなキーと値で設定する必要があります。
