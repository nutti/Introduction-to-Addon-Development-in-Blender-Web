...
アドオン開発者は新しいカテゴリを自由に追加することができますが、既存のいずれかのカテゴリに含まれるように ```category``` を設定し、基本的に新しいカテゴリは作らないようにしましょう。実際、既存のカテゴリだけでも十分カテゴリとしては網羅できているはずですので、新たにカテゴリを作る必要がないと思います。
