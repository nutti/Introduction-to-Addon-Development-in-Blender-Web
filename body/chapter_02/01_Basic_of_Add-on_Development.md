...
[1-4節](../chapter_01/04_Install_own_Add-on.md) で紹介したアドオンは、アドオンの有効化・無効化時にコンソールへメッセージを出力する機能だけを持つアドオンでした。
...
続いて ```print()``` 関数が呼ばれますが、 ```print()``` 関数は引数に指定した文字列を *コンソール* に表示する関数です。
