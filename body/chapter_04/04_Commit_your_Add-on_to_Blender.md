...
###### アドオンを多くの人に使ってもらう方法として[4.3節](03_Publish_your_Add-on.md)では、作成したアドオンをBlender本体に取り込んでもらうことが最良の方法であることを紹介しました。Blenderに作成したアドオンが取り込まれるとアドオンのユーザが多くなる可能性がありますが、アドオンをBlender本体に取り込むまでに必要な作業は非常に多いです。本書最後の節では、作成したアドオンを公開する方法を紹介します。

## アドオンがBlender本体に取り込まれる条件

[1.2節](../chapter_01/02_Use_Blender_Add-on.md) ではアドオンのサポートレベルについて説明しましたが、作成したアドオンをBlender本体に取り込んでもらうためには *Release* または *Contrib* のサポートレベルになる必要があります。
また、 *Contrib* のサポートレベルではビルド時のBlenderのみしかインストールされないため、公式にリリースされたBlenderへ取り込んでもらうためには、 *Release* のサポートレベルになる必要がります。
...
ちなみに *Release* または *Contrib* のサポートレベルであるアドオンは以下から参照できます。
...
なお、1行の最大文字数が79文字を超えてはならないpep8-80というのもありますが、あくまで推奨であってここまでは求められていないようです。
...
コーティング規約のみならず、Blender本体に取り込まれるアドオンは処理も効率的でなくてはなりません。
アドオンを使う人にとっては、少しでも速く処理が終わったほうが良いはずです。
Blenderはアドオン開発に対し、効率的なコーディングをするために必要なベストプラクティスと呼ばれる記事を用意しています。
ベストプラクティスおよびコーディング規約を参照しながらコーディングしましょう。
...
アドオンの作成が完成して、コーディング規約に対応できたらいよいよ次はアドオンのレビューを受けます。
...
|Assigned To|タスクの担当者を指定します。B.P.O内に知り合いがいる場合は、追加するとよいと思います。レビューに参加してもらえるかもしれません。ここで無理して追加しなくてもタスクを見た人がレビューしてくれますので、知り合いがいない場合は何も追加しなくても良いです。ちなみに筆者自身もB.P.Oに登録しているので、タスクを割り当てていただければレビューできるかもしれません。（ユーザ名：Nutti）|
|CC|自分自身を含む開発関係者を追加します。|
|Priority|タスクの緊急度を設定します。新規機能はNormalを指定します。既存の機能のバグ修正であれば、高め優先度を指定しても良いかもしれません|
...
レビュワーの意見を聞くことで、このアドオンはサポートもしっかりしてるし今後も期待できそうだとレビュワーが思うようになり、作成したアドオンに対する支持者が増えていきます。
