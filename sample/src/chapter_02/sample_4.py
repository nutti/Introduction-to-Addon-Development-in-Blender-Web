...
class ReplicateObject(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, "サンプル4: 「%s」を複製しました。" % self.src_obj_name)
        print("サンプル4: オペレーション「%s」が実行されました。" % self.bl_idname)
...
def register():
    ...
    print("サンプル4: アドオン「サンプル4」が有効化されました。")


def unregister():
    bpy.types.VIEW3D_MT_object.remove(menu_fn)
    bpy.utils.unregister_module(__name__)
    print("サンプル4: アドオン「サンプル4」が無効化されました。")
