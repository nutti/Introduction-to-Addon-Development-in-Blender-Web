...
class VIEW3D_PT_CustomMenu(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        sub_column.label(text="領域1:")
        sub_column.operator(NullOperation.bl_idname, text="行 1")
        sub_column.operator(NullOperation.bl_idname, text="行 2")
        split = split.split(percentage=0.7)
        column = split.column()
        sub_column = column.column(align=True)
        sub_column.label(text="領域2:")
        sub_column.operator(NullOperation.bl_idname, text="行 1")
        sub_column.operator(NullOperation.bl_idname, text="行 2")
        split = split.split(percentage=1.0)
        column = split.column()
        sub_column = column.column(align=True)
        sub_column.label(text="領域3:")
        sub_column.operator(NullOperation.bl_idname, text="行 1")
        sub_column.operator(NullOperation.bl_idname, text="行 2")
