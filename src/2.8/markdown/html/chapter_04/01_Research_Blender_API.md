...
なお、ソースコードを参照して処理を流用する場合は、[4-4節](04_Determine_License.html) で説明するアドオンのソースコードに適用されているライセンスに気をつけてください。
...
[2-2節](../chapter_02/02_Register_Multiple_Operators.html) で紹介したように、Pythonコンソールを活用し、Blenderが提供するAPIを検索して実行できます。
...
ただし、他者のソースコードを扱う場合、[4-4節](04_Determine_License.html) で説明するソースコードのライセンスには注意しましょう。
