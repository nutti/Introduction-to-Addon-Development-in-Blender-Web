...
サンプルアドオン『Sample 2-7』のソースコードは、ディレクトリ `sample_2-7` の中に拡張子が `.py` であるファイルが複数あり、ファイル `__init__.py` が含まれています。
このことから、サンプルアドオン『Sample 2-7』は複数のソースコードで構成されるアドオンと判断できます。
