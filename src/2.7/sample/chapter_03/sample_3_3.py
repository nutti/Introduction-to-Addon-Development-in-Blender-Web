...
# @include-source start [update_object_location]
    # オブジェクトの位置を更新
...
# @include-source end [update_object_location]

    def modal(self, context, event):
        props = context.scene.moi_props

# @include-source start [handle_non_timer_event]
        # タイマイベント以外の場合は無視
        if event.type != 'TIMER':
            return {'PASS_THROUGH'}
# @include-source end [handle_non_timer_event]

        # 3Dビューの画面を更新
        if context.area:
            context.area.tag_redraw()

# @include-source start [stop_moving_object]
        # オブジェクトの移動を停止
        if props.running is False:
            ...
            for obj, loc in self.__orig_obj_loc.items():
                obj.location = loc
            return {'FINISHED'}
# @include-source end [stop_moving_object]
...
# @include-source start [store_obj_loc]
            # 開始ボタンが押された時の処理
            if props.running is False:
                self.__orig_obj_loc = {
...
# @include-source end [store_obj_loc]
                props.running = True
