...
# @include-source start [define_properties]
# プロパティ
...
# @include-source end [define_properties]
...
# @include-source start [delete_face]
        # 右クリックされた面を削除
        if props.right_mouse_down is True and props.deleted is False:
# @include-source end [delete_face]
...
# @include-source start [select_clicked_face]
            # クリックされた面を選択
            ...
            if ret == {'PASS_THROUGH'}:
                ...
                return {'PASS_THROUGH'}
# @include-source end [select_clicked_face]
# @include-source start [get_selected_face]
            # 選択面を取得
            ...
            if not isinstance(e, bmesh.types.BMFace):
                ...
                return {'PASS_THROUGH'}
# @include-source end [get_selected_face]
# @include-source start [delete_selected_face]
            # 選択面を削除
            bm.select_history.remove(e)
            bmesh.ops.delete(bm, geom=[e], context=5)
# @include-source end [delete_selected_face]
# @include-source start [update_bmesh]
            # bmeshの更新
            bmesh.update_edit_mesh(me, True)
# @include-source end [update_bmesh]
# @include-source start [post_process]
            # 削除面数をカウントアップ
            props.deleted_count = props.deleted_count + 1
            # マウスクリック中に連続して面が削除されることを防ぐ
            props.deleted = True
            print("サンプル3-1: 面を削除しました。")
# @include-source end [post_process]

# @include-source start [clear_restrict_status]
        # マウスがクリック状態から解除された時に、削除禁止状態を解除
        if props.right_mouse_down is False:
            props.deleted = False
# @include-source end [clear_restrict_status]
...
def register():
    ...
    sc = bpy.types.Scene
# @include-source start [register_properties]
    sc.dfrc_props = PointerProperty(
        name="プロパティ",
        description="本アドオンで利用するプロパティ一覧",
        type=DFRC_Properties
    )
# @include-source end [register_properties]
    print("サンプル3-1: アドオン「サンプル3-1」が有効化されました。")


def unregister():
# @include-source start [unregister_properties]
    del bpy.types.Scene.dfrc_props
# @include-source end [unregister_properties]
