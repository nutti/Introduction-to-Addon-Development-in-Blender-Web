...
# @include-source start [import_aud]
import aud
# @include-source end [import_aud]
...
# @include-source start [set_volume]
# 音量を設定
def set_volume(self, value):
    self['paf_volume'] = value
    if AudioDevice.handle is not None:
        AudioDevice.handle.volume = value
# @include-source end [set_volume]


# @include-source start [get_volume]
# 設定されている音量を取得
def get_volume(self):
    return self.get('paf_volume', 0.5)
# @include-source end [get_volume]
...
# @include-source start [filter_glob]
    # 検索フィルタ
    filter_glob = StringProperty(
        default="*.wav;*.mp3",
        options={'HIDDEN'}
    )
# @include-source end [filter_glob]

# @include-source start [select_audio_file_execute]
    def execute(self, context):
        ...
        return {'FINISHED'}
# @include-source end [select_audio_file_execute]
...
# @include-source start [stop_audio_file]
    def execute(self, context):
        ...
        return {'FINISHED'}
# @include-source end [stop_audio_file]
...
class VIEW3D_PT_PlayAudioFileMenu(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if AudioDevice.handle is not None:
# @include-source start [check_play_status]
            # 再生中の状態
...
# @include-source end [check_play_status]
        else:
            # ファイルブラウザを表示する
            layout.operator(
                SelectAudioFile.bl_idname, text="オーディオファイルを選択",
...
def init_props():
    sc = bpy.types.Scene
# @include-source start [prop_volume]
...
# @include-source end [prop_volume]
